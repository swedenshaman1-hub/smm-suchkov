...
Примени метод Joanna Wiebe только как архитектуру сообщения до написания текста.
...
ЦЕНТРАЛЬНЫЙ ТЕЗИС: одно предложение, прямо отвечающее на вопрос темы
ОБЕЩАНИЕ ЧИТАТЕЛЮ:
МЕХАНИЗМ: одно короткое наблюдаемое различие, на котором держится ответ
ЛОГИЧЕСКИЙ ХОД: три коротких шага без схемы «проблема — объяснение — рефрейм — вопрос»
АРХИТЕКТУРА: выбери одну — тезис и доказательство / конкретная деталь и раскрытие /
ошибка восприятия и уточнение / авторская позиция и следствие / короткий разбор без вопроса
АЛЬТЕРНАТИВНАЯ АРХИТЕКТУРА: выбери другой пункт из того же списка
ЧЕГО НЕЛЬЗЯ ПРИПИСЫВАТЬ ЧИТАТЕЛЮ:
Не предлагай хуки, сцены, метафоры, финал или речевой стиль. Не описывай
реальную аудиторию без исследований и не пиши готовый пост.""",
        "angles": common + """
...
Ты представляешь Дмитрия как эксперта и автора. Используй его собственную речь,
явно одобренные тексты и прямую обратную связь. Старые AI-черновики не считай
образцом.

Сначала найди в источниках только ту экспертную мысль, которая прямо относится
к рабочей теме. Не достраивай психологическую причину по аналогии. Если прямой
опоры нет, напиши: ЭКСПЕРТНАЯ ОПОРА: в источниках нет ответа на эту тему.
Не позиционируй Дмитрия как соматического терапевта и не переноси в новую тему
телесные, энергетические или терапевтические объяснения.

Затем извлеки: длину и ритм фраз, синтаксис, разговорную лексику, дистанцию с
читателем, степень прямоты, переходы между абзацами и характер начала/финала.
Не переноси неподходящие факты, клиентские истории и соседние темы. Не пиши
пример поста. Не предлагай копировать отдельный сленг, междометия, слова
...
Ответ строго:
ЭКСПЕРТНАЯ ОПОРА: один подтверждённый тезис в одном предложении
СТАТУС ОПОРЫ: SUPPORTED / HYPOTHESIS / OBSERVATION / NO_SUPPORT
ИСТОЧНИК УВЕРЕННОСТИ: конкретный тип источника из блокнота или «не указан»
ГРАНИЦА ЗНАНИЯ:
3–5 КОРОТКИХ ПРАВИЛ ГОЛОСА:
3 РЕЧЕВЫХ ШАБЛОНА ПОД ЗАПРЕТОМ:
Не предлагай хук, композицию, пример, метафору или финал.
Если мысль или признак нельзя подтвердить источниками, не включай его.""",
        "human_text": common + """

Примени метод Ann Handley только как набор правил человеческого языка.
Не создавай психологический тезис, не меняй факты, этическую границу и
не предлагай композиционное решение. Выдели приёмы, которые помогут говорить с одним
умным живым человеком, а не с безликим сегментом. Ответ строго:
ПРИНЦИП ИЗ ИСТОЧНИКОВ:
3–5 ПРАВИЛ ЖИВОГО ЯЗЫКА:
ПЯТЬ НЕЕСТЕСТВЕННЫХ КОНСТРУКЦИЙ ПОД ЗАПРЕТОМ:
ОДИН ПРИЗНАК ЛОЗУНГОВОГО ФИНАЛА, КОТОРОГО НУЖНО ИЗБЕЖАТЬ:
Не предлагай вход, финал, психологическое объяснение, сцену или метафору.
...
Предложи ровно три первые фразы с разной механикой: прямая позиция, конкретная
деталь без выдуманной истории и наблюдение. Вопрос допустим только если без него
нельзя точнее войти в тему. Не повторяй исходную тему или её первые
...
тему. Ответ строго:
ВХОД 1 — ПРЯМАЯ ПОЗИЦИЯ:
ВХОД 2 — КОНКРЕТНАЯ ДЕТАЛЬ:
ВХОД 3 — НАБЛЮДЕНИЕ:
РЕКОМЕНДОВАННЫЙ ВХОД:
МЕХАНИКА РЕКОМЕНДАЦИИ:
Не развивай тезис, не предлагай структуру, метафору или финал.""",
