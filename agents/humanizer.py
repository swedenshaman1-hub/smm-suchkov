...
ЭТАЛОН ГОЛОСА ДМИТРИЯ (так должен звучать текст после твоей работы):
...
Признаки живого голоса Дмитрия:
- Риторический вопрос открывает пост — втягивает без давления
- Тире вместо объяснений («это родовые петли — невидимые нити»)
- Парадокс или переворот («ответственность — это свобода»)
- Конкретная боль без абстракций («невозможность расслабиться даже во сне»)
- Финал оставляет вопрос, не закрывает мысль насильно
