...
- преимущественно безличная форма. Слово «вы» запрещено. Обращение на «ты»
  допустимо только в одном финальном проверочном вопросе;
...
2. Хук новый, а не скопированная тема.
...
6. Преимущественно безличная форма, без «вы»; одно «ты» допустимо только
   в финальном вопросе. Нейтральный пол и цельный речевой регистр без
...
2. в тексте нет «вы», а «ты» встречается только в одном финальном вопросе;
3. художественный пример не выдан за реальный случай или общий закон;
4. нет скрытого мотива, предрешённого расставания и попытки «отсрочить финал»;
5. нет ярлыков про репетицию прощания с собой, источник боли и заслуживание
   тепла, любви или принятия;
6. нет кавычек, тире, дефисов и дежурной психологической лексики.
Верни только готовый пост."""
...
def _finalize_voice_brief(brief: str) -> str:
    ...
    brief = re.sub(
        r"(?im)^4\..*$",
        "4. дистанция с читателем: безличная форма; без «вы»; одно "
        "обращение на «ты» допустимо только в финальном вопросе",
        brief,
    )
    brief = re.sub(
        r"(?im)^5\..*$",
