...
- не придумывай признаки охлаждения отношений: короткие разговоры, исчезнувшее
  тепло, неясные планы, молчание и дистанцию, если их нет во входе;
...
5. Преимущественно безличная форма, без «вы»; одно «ты» допустимо только
...
1. в тексте нет «вы», а «ты» встречается только в одном финальном вопросе;
2. не добавлены признаки поведения партнёра, которых нет в исходной теме;
3. нет скрытого мотива, предрешённого расставания и попытки «отсрочить финал»;
4. нет кавычек, тире, дефисов и дежурной психологической лексики.
Верни только готовый пост."""
...
def structural_warnings(topic: str, text: str) -> list[str]:
    ...
    invented_relationship_evidence = {
        r"\bразговор\w*\s+станов\w*\s+короче\b": "короткие разговоры",
        r"\b(?:общ\w*\s+)?план\w*\s+теря\w*\s+ясност\w*\b": "неясные планы",
        r"\b(?:привычн\w*\s+)?тепл\w*.{0,40}\bстал\w*\s+меньше\b": "исчезнувшее тепло",
        r"\bтревожн\w*\s+тишин\w*\b": "тревожная тишина",
    }
    for pattern, label in invented_relationship_evidence.items():
        if not re.search(pattern, topic_lower) and re.search(pattern, lowered):
            warnings.append(
                f"убери придуманный признак охлаждения отношений ({label}): "
                "этого нет в теме"
            )
